...
基于 FastAPI + WebSocket 的四人麻将对战游戏。当前版本 **beta 0.0.3**：单机模式（1 真人 + 3 机器人）、冒险模式（章节/剧情/番种解锁）与联机对战均可用。含账号系统、数据统计、论坛（仅 chinkaku 开放）。
...
- **冒险关卡1-3「正式比赛」(进行中)**：对家Boss机制——打完14张后再摸牌时按概率(win_chance)和牌；和牌由生成器从剩余牌池构造(门清 / 门清+喜相逢 / 门清+单吊字，随机三选一)，分数≤12分，对手番种不受主角解锁限制；上/下家改名为"摸打机器人"、对家"对手"且名字旁显示累计分（`adventure_boss`/`bot_names`，`_gen_boss_hand` 生成器）。过关条件/剧情待定
