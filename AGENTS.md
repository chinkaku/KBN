...
- **剧情杀与跳关**：1-4-1 输（正常）→ 播中间剧情（都茂教混全带幺）→ 自动进 1-4-2；**1-4-1 赢 → 直接通关**（跳关）+ 解锁隐藏关 **1-6**（`hidden_unlock` 配置，`unlocked_hidden` 进度字段，解锁前冒险页完全不可见）；通关（两段任一胜利）→ **+3000金币**（`coins` 配置，账号新增金币字段 `users.json`，冒险页顶部金币栏，`/api/coins` GET/POST）+ 解锁最终章 **1-5** + 保留混全带幺
