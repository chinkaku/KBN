...
- **摸打机器人出牌偏好(分阶段)**：前10巡 85%按 字牌 > 幺九(1/9) > 中张(2-8) 优先打出（字牌/幺九回流牌池，帮玩家凑五门齐/番牌刻）；10巡后变难——40%留住字牌不打(打非字牌)、10%清理字牌(打一张字牌)、50%正常摸切（`_choose_bot_discard(hand, discard_count)`，引擎与房间定时器共用）
