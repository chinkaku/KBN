...
- **冒险关卡1-3「正式比赛」完成**：对家Boss**花桥上田**（`bot_names` 命名，`rooms._is_bot_name` 同步）——每局在 **14~20 巡之间的随机巡数和牌**（`adventure_boss.win_between`）；和牌由 `_gen_boss_hand` 生成器从剩余牌池（全牌−弃牌−其他家手牌）构造 **门清 / 门清+喜相逢 / 门清+单吊字**，分数≤12分，**对手番种不受主角解锁限制**（计分用空锁）
